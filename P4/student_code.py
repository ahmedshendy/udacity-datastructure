...
class PathDetails:
    def __init__(self, path, cost, est_distance):
        self.path = path
        self.cost = cost
        self.est_distance = est_distance
        self.total = self.cost + self.est_distance
        self.edge_point = self.path[-1]
...
def shortest_path(M, start, goal):
    ...
    end_point = M.intersections[goal]
    est_distance = calculate_distance(start_point, end_point)
    path = PathDetails([start], 0, est_distance)
    heapq.heappush(paths, (path.total, path))
    visited_points[start] = 0

    while paths:
        current_path = heapq.heappop(paths)[1]

        # exit if the total cost is greater than the current cost to the goal
        if current_path.total > shortest[1]:
            break
        if current_path.edge_point == goal:
            if current_path.cost < shortest[1]:
                shortest = (current_path.path, current_path.cost)
            continue

        current_road = M.roads[current_path.edge_point]
        current_intersection = M.intersections[current_path.edge_point]

        for point in current_road:
            new_path = [i for i in current_path.path]
            new_path.append(point)
            new_intersection = M.intersections[point]
            new_cost = calculate_distance(
                current_intersection, new_intersection)
            new_cost += current_path.cost

            if point in visited_points and new_cost > visited_points[point]:
                continue

            new_est_distance = calculate_distance(new_intersection, end_point)
            path_details = PathDetails(new_path, new_cost, new_est_distance)
            heapq.heappush(paths, (path_details.total, path_details))
            visited_points[point] = new_cost
...
mapM = MapN(intersections, roads)
shortest_path(mapM, 8, 24)
